 ...
 * 
 * 컬럼명 AS "별칭" : 별칭 띄어쓰기 O, 특수문자O, 문자만 O
 * 
 * AS는 생략가능
 */
 ...
 * 주의사항 2) DISTINCT 구문은 SELECT제일 앞에 작성되어야 한다
 */
...
-- 사번, 이름, 급여, 전화번호 조회
SELECT EMP_NAME ,EMP_NAME ,SALARY ,PHONE 
FROM EMPLOYEE
...
-- NOT BETWEEN A AND B : A 이상 B이하가 아닌 경우 -> A미만 B초과
SELECT EMP_ID ,EMP_NAME ,SALARY 
FROM EMPLOYEE
...
-- EMAIL에서 _ 앞에 글자가 세글자인 사원 조회
SELECT EMP_NAME ,EMAIL 
FROM EMPLOYEE 
WHERE EMAIL LIKE '____%';
-- 문제점 : _를 기준점으로 삼았으나 와일드카드 '_'와 동일한 표기법으로 작성되어 구분되지 않음
-- 해결방법 : LIKE의 ESCAPE OPTION을 이용하여 _를 구분한다

-- LIKE의 ESCAPE OPTION : 일반문자로 처리할 '_'/'%'앞에 아무 특수기호를 첨부해서 구분하게 함
SELECT EMP_NAME ,EMAIL 
FROM EMPLOYEE 
...
-- 부서코드가 'D9' 또는 'D6'이고 -> AND 가 OR보다 우선순위가 높다, ()사용 가능
-- 입사일인 1990-01-01~2000-12-31 이고
-- 급여가 270만 이상인 사원의
