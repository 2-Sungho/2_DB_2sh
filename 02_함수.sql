...
-- EMPLOYEE 테이블에서 사원명,이메일 중 ID만 조회
...
SELECT TRIM ('     H E L L O      ') -- 양쪽 공백 제거
FROM DUAL ; 
...
SELECT ROUND(123.456,0) FROM DUAL ; --123 , 소수점 첫번째 자리에서 반올림(0 기본값)
SELECT ROUND(123.456,-1) FROM DUAL ; --120 , 1의 자리에서 반올림해서 10자리부터 표현
...
SELECT TRUNC(123.456,-1) FROM DUAL ; -- 120 1의 자리 아래 버림 
...
SELECT TO_CHAR(1000000)||'원' FROM DUAL ; 
...
SELECT SYSDATE FROM DUAL ; -- 2022-09-01 11:20:58.000

--2022/09/01 11:20:58 목요일
...
-- EMPLOYEE 테이블에서 각 사원이 태어난 생년월일 조회(1990년 05월 31일) 조회
SELECT EMP_NAME ,
	TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,INSTR(EMP_NO,'-',-1)-1),'RRMMDD'),'YYYY"년" MM"월" DD"일"') AS 생년월일
FROM EMPLOYEE ;
...
SELECT TO_NUMBER('1,000,000','9,999,999')+50000 FROM DUAL ; 
...
-- 급여 최소값, 가장 빠른 입사일, 알파벳 숫자가 가장 빠른 이메일
SELECT MIN(SALARY), MAX(HIRE_DATE), MIN(EMAIL)  
FROM EMPLOYEE ;
-- 급여 최대값, 가장 늦은 입사일, 알파벳 숫자가 가장 느린 이메일
SELECT MAX(SALARY), MIN(HIRE_DATE), MAX(EMAIL)  
FROM EMPLOYEE ;
...
SELECT DISTINCT DEPT_CODE -- 7행 
FROM EMPLOYEE e ;
SELECT COUNT(DISTINCT DEPT_CODE) -- 6행
FROM EMPLOYEE e ;
