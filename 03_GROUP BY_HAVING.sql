...
FROM EMPLOYEE
GROUP BY DEPT_CODE --> DEPT_CODE가 같은행끼리 하나의 그룹이 됨
ORDER BY DEPT_CODE ;
-- ORA-00937: 단일 그룹의 그룹 함수가 아닙니다

-- EMPLOYEE 테이블에서
-- 직급코드가 같은 사람의 급여 평균, 인원 수를
-- 직급코드 오름차순으로 조회
...
SELECT DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여') 성별, 
	...
	SUM(SALARY) "급여 합"
FROM EMPLOYEE e
GROUP BY DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여') -- 별칭사용 X -> 아직 별칭 해석안됨 GROUP BY절에는 별칭 작성 불가능
ORDER BY "인원 수" ; -- 별칭 해석
...
-- EMPLOYEE 테이블에서 직급코드 별 2000년도 이후 입사자들의 급여 합을 조회
-- 직급코드 조회
...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE --> DEPT_CODE 로 그룹을 나누고, 나눠진 그룹 내에서 JOB_CODE 로 또 그룹을 분류
...
-- EMPLOYEE 테이블에서 직급별 인원 수가 5명 이하인 직급코드, 인원 수 조회
-- 직급코드 오름차순
SELECT JOB_CODE , COUNT(*) 
FROM EMPLOYEE 
GROUP BY JOB_CODE 
HAVING COUNT(*)<=5 -- HAVNIG절에는 GROUP함수가 반드시 작성된다
ORDER BY JOB_CODE ;
 ...
 * 
 * - MINUS(차집합) : A에서 A,B교집합 부분을 제거하고 조회
 * 
 * */
