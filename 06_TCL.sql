...
/* TRANSACTION이란?
 ...
 EX) INSERT 수행 --------------------------------> DB 반영(X)
    ...
    2) ROLLBACK : 메모리 버퍼(트랜잭션)에 임시 저장된 데이터 변경 사항을 삭제하고
                 마지막 COMMIT 상태로 돌아감(DB에 변경 배용 반영X)
...
-- INSERT 확인
SELECT * FROM DEPARTMENT2 d ;
--> DB에 반영된 것 처럼 보이지만 SQL 수행 시 트랜잭션 내용도 포함해서 수정된다.
-- 실제로 아직 DB 반영 X
...
SELECT * FROM DEPARTMENT2 d ;
ROLLBACK;
SELECT * FROM DEPARTMENT2 d ; --> ROLLBACK 안됨!
...
--SP2지점까지 ROLLBACK
ROLLBACK TO SP2;
SELECT * FROM DEPARTMENT2 d ; -- 개발6팀만 ROLLBACK

ROLLBACK TO SP1;
SELECT * FROM DEPARTMENT2 d ; -- 개발5팀만 ROLLBACK

ROLLBACK;
SELECT * FROM DEPARTMENT2 d ; -- 개발4팀 ROLLBACK