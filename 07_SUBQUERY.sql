...
ORDER BY JOB_CODE ;
-- SELECT절에 없는 컬럼이라도 FROM,JOIN으로 인해 존재하는 컬럼이면 ORDER BY절에 사용 가능
...
-- ** 해석 순서가 기존 서브쿼리와 다르게
-- 메인쿼리 1행진행 -> 1행에 대한 서브쿼리
-- 메인쿼리 2행진행 -> 2행에 대한 서브쿼리
-- ...
...
ORDER BY HIRE_DATE;
-- 1) 메인쿼리의 1행의 DEPT_CODE를 서브쿼리에 대입
-- 2) 서브쿼리를 수행
-- 3) 서브쿼리의 결과를 이용해서 메인쿼리의 조건절을 수행
...
SELECT EMP_NAME , JOB_CODE , SALARY , (SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE SUB WHERE SUB.JOB_CODE=MAIN.JOB_CODE) 
FROM EMPLOYEE MAIN ;
...
-- SELECT, WHERE, ORDERY BY절 사용 가능
SELECT ROWNUM, EMP_NAME , SALARY
FROM EMPLOYEE
WHERE ROWNUM<=5
ORDER BY SALARY DESC;
--> SELECT 해석 순서 때문에 급여 상위 5명이 아니라 조회 순서 상위 5명의 급여 순위가 조회됨

-- 1) 이름, 급여를 급여 내림차순으로 조회한 결과를 인라인뷰로 사용
-- 2) 메인쿼리 조회 시 ROWNUM 5이하까지 조회
SELECT ROWNUM, EMP_NAME , SALARY
