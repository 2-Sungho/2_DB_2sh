...
-- TCL(TRANSACTION CONTROL LANGUAGE) : 트랜잭션 제어언어(DML수행내용 COMMIT,ROLLBACK)
...
/* 자료형
    NUMBER : 숫자형(정수, 실수)
    CHAR(크기) : 고정길이 문자형 (2000BYTE) -- 학번,주민등록번호와 같이 고정된 형식일 때 사용(고정길이)
        -> ex) CHAR(10) 컬럼에 'ABC' 3BYTE 문자열만 저장해도 10BYTE 저장공간을 모두 사용. 
        
    VARCHAR2(크기) : 가변길이 문자형 (4000 BYTE) -- 글자수가 정해지지 않았을 때 사용(가변길이)
...
-- SQL 작성법 : 대문자 작성 권장, 연계된 단어 사이에는 _(언더바) 사용
...
INSERT INTO "MEMBER" VALUES ('MEM01','123ABC','홍길동','991225-1234567',DEFAULT);
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER" VALUES ('MEM02','456DEF','콩자반','951015-1234567',SYSDATE);

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO "MEMBER" VALUES ('MEM03','789GHI','철수철','980225-1234567',DEFAULT);

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO "MEMBER"(MEMBER_ID,MEMBER_PWD,MEMBER_NAME)
VALUES ('MEM04','846BZC','피치련');

-- ** JDBC에서 날짜를 입력받았을 때 삽입하는 방법 **
-- '2022-09-13 17:33:27'
INSERT INTO "MEMBER" VALUES('MEM05','12F1J2','둘리','940420-1234567',
...
INSERT INTO MEMBER2 VALUES ('MEM01','PASS01','길동이',761616876);
INSERT INTO MEMBER2 VALUES ('MEM02','PASS02','길순이2',0101616876);
...
--  ORA-01400: NULL을 ("KH_LSH"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- 테이블에 대한 식별자(IDENTIFIER) 역할을 함 EX) 학번
...
CREATE TABLE USER_USED_FK(
  ...
  -- 테이블 레벨(FOREIGN KEY 단어는 테이블레벨에서만 사용!)
...
CREATE TABLE USER_USED_FK2(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK2 REFERENCES USER_GRADE2 ON DELETE SET NULL --삭제옵션
...
-- CHECK는 작성 후 수정이 번거롭지만 , FK는 부모테이블에 행을 추가,수정,삭제하는게 간편
...
--> 서브쿼리로 테이블 생성 시
-- 테이블의 형태(컬럼명,데이터타입)+NOT NULL 제약조건만 복사되고
